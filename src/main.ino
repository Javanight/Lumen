...
void updateState(JsonObject& json) {
  if (sizeof(json["data"]) > 0) {
    ...
    daytime = json["data"][jsonIteration++];

    for (int i = 0; i < NUM_OF_COLOR_COMPONENT; i++) {
      colorComponents[i] = json["data"][jsonIteration++];
    }
  }
}
...
void sendState() {
  ...
  response += String(daytime);

  for (int i = 0; i < NUM_OF_COLOR_COMPONENT; i++) {
    if (element++ > 0) { response += ","; }
    response += String(colorComponents[i]);
  }
 ...
 * actions.js
 */
void handleDaytimeJs() {
  Serial.println(server.uri());
  sendFile("/daytime.js", "application/json");
}
...
void setup(void){
  ...
  server.on("/daytime.js",  handleDaytimeJs);
